/*
 * Furthest Points Toy 2 high based on Nearest Points Toy 2
 *
 * Authors:
...
//		std::cerr << "\n";
		...
		{
			tlist.push_back(furthest_time(c2(k2_roots[i]), c1, dc1, cd1));
		}

		for ( unsigned int i = 0; i < dk2_roots.size(); ++i )
		{
			tlist.push_back(furthest_time(c2(dk2_roots[i]), c1, dc1, cd1));
		}
	...
	void operator() ()
	{
		//furthest_times_impl( tlist.size() / 2, 0, tlist.size() - 1 );
		furthest_times_impl();
		d = sqrt(dsq);
	}
...
private:
	void furthest_times_impl()
	{
		...
		for ( unsigned int i = 1; i < tlist.size(); ++i )
		{
			to = tlist[i];
			t = from + (to - from) / 2 ;
			std::pair<double, double> fpc = loc_furthest_times(t, from, to);
