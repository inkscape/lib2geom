...
Geom::Path2::Path Gear::path() {
    ...
    for (int i=0; i < _number_of_teeth; i++)
    {
        ...
        pb.append(Path2::SBasisCurve(trailing_I));
        prev = trailing_I.at1();
        ...
        if (base_radius() > root_radius()) {
            ...
            pb.appendNew<Path2::LineSegment>(trailing_end);
            prev = trailing_end;
        }
    }
