...
{
public:
	fp_finder(cairo_t* _cr, D2<SBasis> const& _c1, D2<SBasis> const& _c2)
...
//		}
		...
		// add cuts at furthest points to the other curve cuts points
		cuts1.clear();
		cuts1.reserve(c1.size()+1);
		for ( unsigned int i = 0; i < c1.size(); ++i )
		{
			cuts1.push_back( furthest_time(c1[i].at0(), _c2, dc2, cd2) );
		}
		cuts1.push_back( furthest_time(c1[c1.size()-1].at1(), _c2, dc2, cd2) );
...
//		cairo_line_to(cr, c2(cuts1[c1.size()]));
		...
		{
			cuts2.push_back( furthest_time(c2[i].at0(), _c1, dc1, cd1) );
		}
		cuts2.push_back( furthest_time(c2[c2.size()-1].at1(), _c1, dc1, cd1) );
...
//		}
		...
		// add cuts at furthest points to max and min curvature points 
		// of the other curve
		cuts1.clear();
		cuts1.reserve(dk2_roots.size());
		for ( unsigned int i = 0; i < dk2_roots.size(); ++i )
		{
			cuts1.push_back(furthest_time(_c2(dk2_roots[i]), _c1, dc1, cd1));
		}
...
//		cairo_stroke(cr);
		...
		for ( unsigned int i = 0; i < pwc1.size(); ++i )
		{
			p = pwc1[i](0.5);
			nv = n1[i](0.5);
			fpt = furthest_time(p, _c2, dc2, cd2);
			if( dot( _c2(fpt) - p, nv ) > 0 )
			{
				if ( dot( nv, n2(fpt) ) > 0 )
				{
					n1[i] = -n1[i];
		...
		for ( unsigned int i = 0; i < pwc2.size(); ++i )
		{
			p = pwc2[i](0.5);
			nv = n2[i](0.5);
			fpt = furthest_time(p, _c1, dc1, cd1);
			if( dot( _c1(fpt) - p, nv ) > 0 )
			{
				if ( dot( nv, n1(fpt) ) > 0 )
				{
					n2[i] = -n2[i];
...
private:
	...
	{		
		...
		for ( unsigned int i = 0; i < c1.size(); ++i )
		{
			...
			std::pair<double, double> fpc = loc_furthest_times(t, c1.cuts[i], c1.cuts[i+1]);
			if ( fpc.second != -1 && dsq > L2sq(c1(fpc.first) - c2(fpc.second)) )
			{
				t1 = fpc.first;
				t2 = fpc.second;
				p1 = c1(t1);
	...
	loc_furthest_times( double t, double from = 0, double to = 1 )
	{
		...
		std::pair<double, double> fp(-1,-1);
		std::pair<double, double> fpf(from, -1);
		std::pair<double, double> fpt(to, -1);
		double ct = t;
		double pt = -1;
		double s = furthest_time(c1(t), cc2, dc2, cd2);
...
//				cairo_line_to(cr, evl2(s));
				...
				s = furthest_time(c1(ct), cc2, dc2, cd2);
...
//				s = s + angle / (dcn2(s) * k2(s));
			...
			{
				++iter1;
				ct = furthest_time(c2(s), cc1, dc1, cd1, from, to);
				s = furthest_time(c1(ct), cc2, dc2, cd2);
			}
			iter = iter1 + iter2;
			//std::cerr << "s: " << s << std::endl;
  ...
  private:
    		   ...
    		   int width, int height, bool save, std::ostringstream *timer_stream) 
    {
        ...
        if( ip_list.empty() )
        {
	        ...
	        cairo_move_to(cr, fp.firstPoint());
	        cairo_line_to(cr, fp.secondPoint());
	        cairo_stroke(cr);
	        //std::cerr << "fp: (" << fp.firstValue() << "," << fp.secondValue() << ")" << std::endl;
        }
