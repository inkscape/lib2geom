...
/*
 *  this is a proof of concept implementation of an svg elliptical arc class
 *  for a possible integration into the 2geom library
 *  there is also a toy class to test and to interact with elliptical arcs
 *
 */
...
class EllipticToy: public Toy 
{
  public:
    ...
    {
        // uncomment this to have a bit of interactivity but you can't modify 
        // large_arc and sweep flags, so when you try to generate an arc 
        // whose length is near to half the length of the whole ellipse 
        // it's probable that an assertion failure is raised because 
        // there is no ellipse that satisfies the given constraints;
        // moreover x and y ray are fixed.
        ...
        Coord cos_rot_angle = std::cos(rot_angle);
        Coord sin_rot_angle = std::sin(rot_angle);



        // draw axes passing through the ellipse center
        cairo_set_source_rgba(cr, 0.0, 1.0, 0.0, 1.0);
        cairo_set_line_width(cr, 0.2);
        double left_coord = cx - rx - 100;
        double right_coord = cx + rx + 100;
        double up_coord = cy - ry - 100;
        double down_coord = cy + ry + 100;
        cairo_move_to(cr, left_coord, cy);
        cairo_line_to(cr, right_coord, cy);
        cairo_move_to(cr, cx, up_coord);
        cairo_line_to(cr, cx, down_coord);
        cairo_move_to(cr, left_coord, up_coord);
        cairo_line_to(cr, right_coord, down_coord);
        cairo_move_to(cr, right_coord, up_coord);
        cairo_line_to(cr, left_coord, down_coord);
        cairo_move_to(cr, 0, 0);
        cairo_stroke(cr);

        // this is useful only with circular arcs
        Point direction = start_point - end_point;
        Coord norm = L2(direction);
        Coord center_dist = std::sqrt(rx * rx - L2sq(direction) / 4);
        direction /= norm;
        Point half_point = (start_point + end_point) / 2;
        Point center1 = center_dist * direction.cw() + half_point;
        Point center2 = center_dist * direction.ccw() + half_point;

        cairo_arc(cr, center1[0], center1[1], rx, 0, 2*M_PI);
        cairo_stroke(cr);
        cairo_arc(cr, center2[0], center2[1], rx, 0, 2*M_PI);
        cairo_stroke(cr);
//        cairo_set_source_rgba(cr, 0.0, 0.0, 0.0, 1.0);
//        cairo_move_to(cr, start_point[0] + 5, start_point[1] + 5);
//        cairo_show_text(cr, "1");
//        cairo_move_to(cr, end_point[0] + 5, end_point[1] + 5);
//        cairo_show_text(cr, "2");
//        cairo_stroke(cr);

        // draw the same arc but with cairo in order to make a comparison
        cairo_matrix_t transform_matrix;
        cairo_matrix_init( &transform_matrix, 
                           rx * cos_rot_angle, rx * sin_rot_angle,
                          -ry * sin_rot_angle, ry * cos_rot_angle,
                           cx,                 cy 
                         );
        ...
        {
            cairo_arc(cr, 0, 0, 1, end_angle, start_angle);
        }
        cairo_restore(cr);
        cairo_stroke(cr);
        ...
        *notify << "< Ellipse Toy >" << " center: " << cx << " , " << cy;
  ...
  public:
    EllipticToy() 
    {
        // change these parameters to get the various possible elliptical arcs
...
unsigned B_bez_ord=3;
    ...
    init( argc, argv, new EllipticToy() );
