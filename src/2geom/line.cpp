...
inline
bool intersection_impl( Crossing & solution,
		                Point const& V1, Point const O1,
		                Point const& V2, Point const O2 )
{
	double detV1V2 = V1[X] * V2[Y] - V2[X] * V1[Y];
	if (are_near(detV1V2, 0)) return false;
	...
	solution.ta = detBV2 * inv_detV1V2;
	solution.tb = detV1B * inv_detV1V2;
...
Crossings intersection_impl(Ray const& r1, Line const& l2, unsigned int i)
{
	...
	bool single_solution
		= intersection_impl( crossings.front(),
		                     r1.versor(), r1.origin(),
		                     l2.versor(), l2.origin() );

	if (single_solution)
	{
		if (crossings.front().ta < 0)
		{
			Crossings no_crossing;
			return no_crossing;
		}
		else
		{
			if (i != 0)
			{
				std::swap(crossings.front().ta, crossings.front().tb);
			}
			return crossings;
		}
	}
		                     ...
		                     unsigned int i )
{
	...
	bool single_solution
		= intersection_impl( crossings.front(),
		                     ls1.finalPoint() - ls1.initialPoint(),
		                     ls1.initialPoint(),
		                     l2.versor(),
		                     l2.origin() );
	...
	if (single_solution)
	{
		if ( crossings.front().getTime(0) < 0
				|| crossings.front().getTime(0) > 1 )
		{
			Crossings no_crossing;
			return no_crossing;
		}
		                     ...
		                     unsigned int i )
{
	...
	if ( are_near(r2.origin(), ls1) )
	{
		...
		{
			crossings.front().ta = crossings.front().tb = 0;
			return crossings;
		}
		else if ( are_near(ls1.finalPoint(), r2.origin()) && eqvs )
		{
			if (i == 0)
			{
				crossings.front().ta = 1;
				crossings.front().tb = 0;
			}
			else
			{
				crossings.front().ta = 0;
				crossings.front().tb = 1;
			}
			return crossings;
		}
		else
...
Crossings intersection(Line const& l1, Line const& l2)
{
	...
	bool single_solution
		= detail::intersection_impl( crossings.front(),
		                             l1.versor(), l1.origin(),
		                             l2.versor(), l2.origin() );
	if (single_solution)
	{
...
Crossings intersection( LineSegment const& ls1, LineSegment const& ls2 )
{
	...
	else if ( are_near(ls2.finalPoint(), ls1) )
	{
		...
		{
			crossings.front().ta = crossings.front().tb = 1;
			return crossings;
		}
...
Line make_angle_bisector_line(Line const& l1, Line const& l2)
{
	...
	Point O = l1.pointAt(crossings.front().ta);
	Point A = l1.pointAt(crossings.front().ta + 1);
	double angle = angle_between(l1.versor(), l2.versor());
	Point B = (angle > 0) ? l2.pointAt(crossings.front().tb + 1)
				          : l2.pointAt(crossings.front().tb - 1);
