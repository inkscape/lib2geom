...
cdef extern from "2geom/curve.h" namespace "Geom":
    cdef cppclass Curve:
        ...
        Coord nearestPoint(Point &, Interval &)
        vector[double] allNearestPoints(Point &, Coord, Coord)
        vector[double] allNearestPoints(Point &, Interval &)
        Coord length(Coord)
...
cdef extern from "2geom/elliptical-arc.h" namespace "Geom":
    cdef cppclass EllipticalArc:
        ...
        double nearestPoint(Point &, double, double)
        int degreesOfFreedom()
        Curve * derivative()
        Curve * transformed(Affine &)
        vector[Point] pointAndDerivatives(Coord, unsigned int)
