...
bool PathIntersectionGraph::_prepareIntersectionLists(Coord precision)
{
    ...
    for (auto & component : _components) {
        for (std::size_t i = 0; i < component.size(); ++i) {
            component[i].xlist.sort(IntersectionVertexLess());
        }
    }
...
void PathIntersectionGraph::_assignComponentStatusFromDegenerateIntersections()
{
    ...
    for (auto & component : _components) {
        for (unsigned li = 0; li < component.size(); ++li) {
            IntersectionList &xl = component[li].xlist;
            bool has_in = false;
            bool has_out = false;
            for (auto & i : xl) {
            ...
            if (has_in && !has_out) {
                component[li].status = INSIDE;
            }
            if (!has_in && has_out) {
                component[li].status = OUTSIDE;
            }
        }
    }
}
...
std::vector<Point> PathIntersectionGraph::intersectionPoints(bool defective) const
{
    ...
    for (std::size_t i = 0; i < _components[0].size(); ++i) {
        for (const auto & x : _components[0][i].xlist) {
            if (x.defective == defective) {
...
void PathIntersectionGraph::fragments(PathVector &in, PathVector &out) const
{
    ...
    for (unsigned w = 0; w < 2; ++w) {
        for (const auto & li : _components[w]) {
