...
struct Vertex {
    ...
    bool operator<(Vertex const & to) const {
        return this < &to;
    }
};
...
Graph sweep_graph(std::vector<Path> const &ps, Dim2 d = X) {
...
public:
    unsigned val;
    other_matches(unsigned v) : val(v) {}
...
class SweepWindow: public Toy {
    ...
    virtual void draw(cairo_t *cr, std::ostringstream *notify, int width, int height, bool save, std::ostringstream *timer_stream) {
        ...
        std::set<Vertex> new_verts;
        std::vector<Vertex*> bnd;
        std::set<unsigned> deleted_sections;
        for(unsigned i = 0; i < output.verts.size(); i++) {
            Vertex &v = output.verts[i];
            v.id = i;
            if(v.enters.size() + v.exits.size() == 1) {
                //std::cout << i << std::endl;
                bnd.push_back(&v);
            } else {
                new_verts.insert(v);
        ...
        while(bnd.size()) {
            Vertex* top = bnd.back();
            bnd.pop_back();
            for(unsigned i = 0; i < top->enters.size(); i++) {
                unsigned idx = top->enters[i].other;
                deleted_sections.insert(top->enters[i].section);
                output.verts[idx].exits.erase(remove_if(output.verts[idx].exits.begin(),
                                                     output.verts[idx].exits.end(),
                                                        other_matches(top->id)));
                if(output.verts[idx].exits.size() + output.verts[idx].enters.size() == 1)
                    bnd.push_back(&output.verts[idx]);
            }
            for(unsigned i = 0; i < top->exits.size(); i++) {
                unsigned idx = top->exits[i].other;
                deleted_sections.insert(top->exits[i].section);
                output.verts[idx].enters.erase(remove_if(output.verts[idx].enters.begin(),
                                                     output.verts[idx].enters.end(),
                                                        other_matches(top->id)));
                if(output.verts[idx].enters.size() + output.verts[idx].exits.size() == 1)
                    bnd.push_back(&output.verts[idx]);
            }
            top->exits.clear();
            top->enters.clear();
        ...
        vector<Section> remaining_sections;
        for(unsigned i = 0; i < output.edges.size(); i++) {
            if(deleted_sections.find(i) != deleted_sections.end()) {
                
            } else {
                //remaining_sections.push_back(output.edges[i]);
                draw_section(cr, output.edges[i], path);
            }
        }
        for(unsigned i = 0; i < output.verts.size(); i++) {
            Vertex &v = output.verts[i];
            v.id = i;
            if(v.enters.size() + v.exits.size() > 0) {
                draw_circ(cr, v.avg);
            }
        }
    ...
    void first_time(int argc, char** argv) {
        const char *path_name="triarrange.svgd";
        if(argc > 1)
            path_name = argv[1];
